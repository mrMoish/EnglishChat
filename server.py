...
def edit_tg_msg_with_mini_lesson(connection_to_telegram, chat_id: int, message_id: int, text: str):
    import urllib.parse
...
def send_msg_with_dont_understand(connection_to_telegram, chat_id: int, text: str):
    import urllib.parse
    reply_markup = urllib.parse.quote(
        '{"inline_keyboard":[[{"text":"не понял вопрос","callback_data":"/dont_understand"}]]}'
    )
    connection_to_telegram.request('GET',
        f'/bot{os.environ.get('BOT_TOKEN')}/sendMessage?chat_id={chat_id}&text={text}&reply_markup={reply_markup}')
...
def translate(message, source_lang, target_lang):
    ...
    print('Translation response:', response, flush=True)
...
# Avoid complex grammar. Keep it simple and catchy. Additional emojis are allowed if they help engagement.
    ...
    connection.request(
        ...
        json.dumps({
            'model': 'openai/gpt-4o',
            'prompt':f'Create a question in {target_lang}, making sure that both the question and the answer contain the following words: {words}. The tone should be friendly, modern, and conversational. Avoid complex grammar. Keep it simple and memorable.',
        }), {
            "Content-Type": "application/json",
            'Authorization': os.environ['OPENROUTER_API_KEY']
        })
    ...
    def do_POST(self):
        ...
        if 'message' in data:
            ...
            if 'text' in data['message']:
                ...
                match message:
                    case '/start':
                        ...
                        send_telegram_message_html(conn, id, encode_url('<b>Начнем!? Напишите фразу на русском или испанском!</b>'))
                    case _:
                        ...
                        edit_tg_msg_with_mini_lesson(conn, id, msg_id, encode_url(tr))
            ...
            if data['callback_query']['data'] == '/lesson':
                ...
                status, data = send_audio_bytes(BOT_TOKEN, id, audio_bytes=audio_bytes, filename="вопрос.mp3")
                send_msg_with_dont_understand(conn, id, encode_url('Прослушай вопрос и напечатайте ответ!'))
                ...
                conn = http.client.HTTPSConnection("api.telegram.org")  # подождать, чтобы сообщение с аудио отправилось
                conn.request(
                    "GET",
    ...
    def do_POST(self):
        ...
        if 'message' in data:
            ...
            if 'text' in data['message']:
                ...
                match message:
                    case '/start':
                        hello_msg = 'Это бот для ежедневных мини-уроков испанского.\n\n  На данный момент этот бот работает в режиме переводчика c любого языка мира на русский и с русского на испанский🇪🇸 🇦🇷\n\nпреимущества:\n✅ сохраняетcя вся ваша история перевода, в удобном формате чата на всех устройствах, где есть телеграм\n\nв будущем:\n🚀 мы добавим мини-уроки, включающие аудиозапись, текст и вопросы, вы сможете каждый день повторять <b>нужные лично вам</b> фразы 🚀🚀🚀'
                        send_telegram_message(conn, id, encode_url(hello_msg))
                        send_telegram_message_html(conn, id, encode_url('<b>Начнем!? Напишите фразу на русском или испанском!</b>'))
                    case _:
